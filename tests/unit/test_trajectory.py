...
class CalculateDiffusionCoefficientTester(unittest.TestCase):
    ...
    def test_calculating_diffusion_coefficient_using_covariance_based_method(self):
        ...
        expected_diffusion_coefficient = 0

        particle_positions = np.empty((5,), dtype=[('frame_index', np.int16), ('time', np.float32), ('position', np.float32)])
        ...
        calculated_diffusion_coefficient = t.calculate_diffusion_coefficient_using_covariance_based_estimator(R=1/4)
